...
| [mcp.md](./mcp.md) | MCP integrators — server config, tools, prompts | English |
...
These live outside `docs/` because they are embedded into the binary via `include_str!` and served via MCP to LLM agents. They follow different writing rules (terse spec, no human prose).

| Path | Served as |
|---|---|
| [../agent-resources/template-authoring.md](../agent-resources/template-authoring.md) | MCP prompt `crud_template_authoring` |
| [../agent-resources/entity-json-guide.md](../agent-resources/entity-json-guide.md) | MCP tool `crud_entity_schema { name: "guide" }` |
